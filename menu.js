...
var fontFamily = "font-family: Arial,\'Helvetica Neue\',Helvetica,sans-serif !important;";
...
var blackBorder = " border-radius: 4px !important; border: 1px solid #000 !important;";
var alertPre = fontFamily + " position: absolute; font-size: 14px; margin: 0 !important; padding: 4px !important; z-index: 99999;";
var alertStyle = alertPre + blackBorder + " color: #000; background-color: #FFD700; border: 1px solid #000; font-weight: normal; font-style: normal;";
var alertBox = fontFamily + blackBorder + " line-height: 10px !important; font-size: 10px !important; margin: 4px !important; padding: 4px !important; z-index: 99999; color: #000 !important; background-color: #FFD700 !important; ";

var alertStyleTight = fontFamily + "color: #000 !important; background-color: #FFD700 !important; border: 1px solid #000 !important; font-weight: normal !important; font-style: normal !important; font-size: 11px !important; margin: 0 !important; padding: 0 !important; z-index: 99999 !important;";
var borderStyle = "border: 3px solid #5cd344; border-radius: 4px !important; padding: 4px !important; z-index: 99998 !important;";
var highlightStyle = "border: 3px solid #5cd344 !important; margin: 4px !important; border-radius: 4px !important; padding: 4px !important; z-index: 99998 !important";
var blackoutStyle = "background-color:#000 !important; color:#000 !important; opacity: 1 !important; border: 3px solid #5cd344 !important; border-radius: 4px !important;";

// CSS Styles for the box

var GUToAFStyle = "#GUToAFDiv {" + fontFamily + " position:fixed; z-index:999998; background-color:#f1f1f1; border:1px solid #d7d7d7; border-radius: 6px; box-shadow: 0px 3px 15px rgba(0,0,0,0.2);top:25px; left:25px;}";
GUToAFStyle += "#GUToAFDivheader {" + fontFamily + lineHeight + " font-size: 14px !important; padding:2px 7px !important; cursor:move; z-index:999999; background-color:#5cd344; color:#000; font-weight:bold; border-top-left-radius: 6px; border-top-right-radius: 6px;}";
GUToAFStyle += "#GUToAFInner {overflow:auto !important;}";
GUToAFStyle += "#GUToAFUl { list-style-type:none !important; list-style-image:none !important; margin:0 !important; padding:0 !important;}";
GUToAFStyle += "#GUToAFUl li {" + boxFont + liPad + " border-top: 1px solid #d7d7d7 !important; font-size:12px !important;}";
GUToAFStyle += "#GUToAFUl li:first-child { border-top: 0 !important; }";
GUToAFStyle += "#GUToAFUl li:hover {" + lineHeight + liPad + "background-color:#d3d3d3 !important;}";
GUToAFStyle += "#GUToAFUl li a:link, #GUToAFUl li a:visited, #GUToAFUl li a:active {" + boxFont + anchorPad + " color:#3359ec !important; text-decoration:none !important;}";
GUToAFStyle += "#GUToAFUl li a:hover {" + lineHeight + anchorPad + "color:#552c9f !important; text-decoration:underline !important;}";
GUToAFStyle += "#GUToAFResults {" + boxFont + "color: #000;  font-size: 12px !important; font-weight: bold !important; margin: 3px 0 !important; padding-left: 7px !important; border-bottom: 2px solid #5cd344 !important; max-height: 6em !important; overflow: auto !important; background-color: #eff !important;}";
GUToAFStyle += "#GUToAFUl li.GUToAFlineAbove { border-top: 2px solid #5cd344 !important; }";
GUToAFStyle += ".GUToAFsummary { " + boxFont + liPad + "border: 2px solid #5cd344 !important; border-radius: 4px; background-color: #fff !important; color: #000 !important; }";

GUToAFStyle += "#GUToAFUlDetails { list-style-type:disc !important; list-style-image:none !important; margin:0 !important; padding:0 !important;}";
GUToAFStyle += "#GUToAFUlDetails li {" + boxFont + " font-weight: normal; padding: 0 !important; margin: 0 0 0 9px !important; font-size:9px !important;}";
...
var linkList = [
...

   {
      ...
      script: "https://gutterstar.bizland.com/whatsock/training/matrices/visual-aria/roles.js",
      removeSelf: "yes"
   },
   {
      key: "GUToAF-reset",
...
function removeDOM() {
   if(document.querySelectorAll("[data-GUToAFset]")) {
      document.querySelectorAll("[data-GUToAFset]").forEach(e => e.parentNode.removeChild(e));
   }
}
...
function showResults(name,count,name2,count2) {
   ...
   document.getElementById("GUToAFUl").insertAdjacentElement('beforebegin',resultsDiv);
}
      ...
      for (var i=0; i < col.length; i++) {
		...
		altList += "<li>";
         if ( col[i].hasAttribute("aria-label") ) {
          alttext = document.createTextNode("aria-label=" + col[i].getAttribute('aria-label'));
...
function showHeadings() {
   ...
   if (col.length > 0)
   {
      ...
      buildCSS("h1,h2,h3,h4,h5,h6,[role='heading']{ " + borderStyle + "}","showHeadings");
   }
...
function showFocusOrder() {
   ...
   function traverseFrames() {
      ...
      var nl = document.querySelectorAll("[tabindex], button, a[href], area, input:not([type=hidden]) , select, textarea, iframe");
   ...
   function initHelper(document, nl) {
      ...
      for (var i = 0; i < nl.length; i++) {
         if (!nl[i].hasAttribute('disabled') && !nl[i].hasAttribute('data-GUToAFLink')) {
   ...
   regions.forEach(function(element) {
      ...
      if (el.length > 0) {
         for (var i = 0; i < el.length; i++) {
            ...
            s.appendChild(t);
            s.style = alertPre + " background-color: " + element.color + "; color: " + element.font + "; font-weight: bold;";
            s.setAttribute("data-GUToAFSet","showRegions");
            el.item(i).parentNode.insertBefore(s, el.item(i));
            CSS += element.css + ", " + thisRole + " { padding: 20px !important;  border-radius: 4px; border: 3px dotted " + element.color + "; }";
            count++;
         }
      }
    });
   ...
   if (col.length > 0)
   {
      for (var i=0; i < col.length; i++) {
         ...
         if (document.getElementById(dbID))
         {
            ...
            destinationNode.setAttribute("data-GUToAFSet","showDescribedByDestination");
            var dNtext = document.createTextNode("ID: " + dbID);
            destinationNode.appendChild(dNtext);
            document.getElementById(dbID).insertAdjacentElement('beforebegin',destinationNode);
         ...
         else
         {
            ...
            parentNode.style = alertStyle;
            parentNode.setAttribute("data-GUToAFSet","showDescribedByParent");
            var pNtext = document.createTextNode("DescribedBy: " + dbID + " INVALID ID");
            parentNode.appendChild(pNtext);
            col[i].parentNode.insertBefore(parentNode, col[i]);
         }
...
function showRoles() {
   ...
   if (el.length > 0) {
      for (var i = 0; i < el.length; i++) {
         s = document.createElement('span');
         t = document.createTextNode("@role='" + el.item(i).getAttribute("role") + "'");
         s.setAttribute("title",el.item(i).nodeName);
         s.appendChild(t);
         s.style = alertStyle;
...
function showAriaLabel() {
   ...
   if (el.length > 0) {
      for (var i = 0; i < el.length; i++) {
         s = document.createElement('span');
         t = document.createTextNode("@aria-label='" + el.item(i).getAttribute("aria-label") + "'");
         s.setAttribute("title",el.item(i).nodeName);
         s.appendChild(t);
         s.style = alertStyle;
   ...
   if (col.length > 0)
   {
      for (var i=0; i < col.length; i++) {
         ...
         if (document.getElementById(dbID))
         {
            ...
            var destinationNode  = document.createElement("span");
            destinationNode.style = alertStyle;
            destinationNode.setAttribute("data-GUToAFSet","showLabelledByDestination");
            var dNtext = document.createTextNode("ID: " + dbID);
            destinationNode.appendChild(dNtext);
            document.getElementById(dbID).insertAdjacentElement('beforebegin',destinationNode);
            var CSS = "#" + dbID + "{ " + highlightStyle + "}";
         ...
         else
         {
            ...
            var pNtext = document.createTextNode("LabelledBy: " + dbID + " INVALID ID");
            parentNode.appendChild(pNtext);
            col[i].parentNode.insertBefore(parentNode, col[i]);
         }
...
function showTables() {
   ...
   tableParts.forEach(function(element) {
      CSS += element.css + " { border: 3px dotted " + element.color + " !important;";
      if (element.bg) {
         CSS += " background-color: " + element.bg + " !important;";
      }
      if (element.pad) {
         CSS += " margin: " + (element.pad * 2) + "px !important; padding: " + element.pad+ "px !important;";
      }
      CSS += "}";
   });
   ...
   if (table.length > 0) {
      for (var t = 0; t < table.length; t++) {
         ...
         tableParts.forEach(function(element) {
            ...
            if (el.length > 0) {
               for (var i = 0; i < el.length; i++) {
                  el.item(i).innerHTML = "<span style=\"" + alertStyleTight + "\" data-GUToAFSet=\"ShowTables\">" + element.title + "</span>" + el.item(i).innerHTML;
               }
            }
         });
         ...
         text += "THEAD:" + theadCount + " | TBODY:" + tbodyCount + " | TFOOT:" + tfootCount + " | TR:" + trCount + " | TH:" + thCount + " | TD:" + tdCount;
...
function cssAriaPresentation() {
   ...
   CSS += createCSSContent("[role=\"presentation\"]","role=Presentation","before");
...
function cssTextspacing() {
   var CSS = "*{line-height:1.5 !important; letter-spacing: 0.12em !important; word-spacing: 0.16em !important; } p {margin-bottom: 2em !important;}";
   buildCSS(CSS,"cssTextSpacing");
}

function cssGrayscale() {
   var CSS = "html { filter: grayscale(100%) !important; -webkit-filter: grayscale(1) !important; }";
...
function GUToAFBuildLinks() {
   ...
   linkList.forEach(function(element) {
      ...
      if (element.script) {
         onclick = "javascript:(function(){var element=document.createElement('script'); element.setAttribute('src','" + element.script +"');document.body.appendChild(element);})();"
      }
      ...
      if (url) {
         ...
         if (element.style) { thisLI.className = element.style + " GUToAFLI"; }
         else { thisLI.className = "GUToAFLI" }
         if (element.desc) { thisLI.title = element.desc; }
